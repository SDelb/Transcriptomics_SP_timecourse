...
```{r}
# Note: some genes cause a problem in cwFC evaluation: cw fold changes are all NA. Will remove these gene from DE gene list. 
DEgenes = DEgenes[,1]
...
for(i in 7:length(timepoints)){
  ...
  toOut <- scMappR_and_pathway_analysis(count_file = tp.cpm, 
                                      ...
                                      max_proportion_change = 10, 
                                        # Maximum cell-type proportion change – may be useful if there are many rare
...
# Keep for each gene the cell type with max specificity (abs value), and set cutoff of 0.5:
